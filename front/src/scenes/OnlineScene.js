class OnlineScene extends Scene {
	...
	init() {
		...
		socket.on("challengeOpponent", (key) => {
			...
			alert(
				`Перший хто пройде за цим посиланням буде грати з вами:\n${location.href}`
			);
		});
	...
	statusUpdate() {
		...
		if (!this.status) {
			...
			statusDiv.textContent = "Пошук соперника";
		} else if (this.status === "play") {
			statusDiv.textContent = this.ownTurn ? "Ваш хід" : "Хід суперника";
		} else if (this.status === "winner") {
			statusDiv.textContent = "Вы перемогли";
		} else if (this.status === "loser") {
			statusDiv.textContent = "Вы програли";
		} else if (this.status === "waiting") {
			statusDiv.textContent = "Чекаємо на суперника";
		}
	}
